...
- 현재 `authorization_code + PKCE` OAuth 로그인 플로우는 아직 구현되어 있지 않습니다.
- 따라서 OAuth discovery는 읽을 수 있어도, 서버가 `authorization_code`만 요구하면 현재 앱만으로는 연결할 수 없습니다.
...
- OAuth protected resource metadata:
  `https://wiki.ss-fai.cloud/.well-known/oauth-protected-resource/mcp`
- OAuth authorization server metadata:
  `https://wiki.ss-fai.cloud/.well-known/oauth-authorization-server/mcp`

실제 확인 결과:

```json
{
  "issuer": "https://wiki.ss-fai.cloud",
  "authorization_endpoint": "https://wiki.ss-fai.cloud/oauth/authorize",
  "token_endpoint": "https://wiki.ss-fai.cloud/oauth/token",
  "revocation_endpoint": "https://wiki.ss-fai.cloud/oauth/revoke",
  "registration_endpoint": "https://wiki.ss-fai.cloud/oauth/register",
...
- `client_credentials`는 지원하지 않음
- `authorization_code + PKCE` 기반 OAuth를 요구함
- 현재 앱은 이 로그인 플로우를 구현하지 않음

추가 테스트:

Outline API key를 Bearer token으로 MCP endpoint에 직접 넣어 `initialize` 호출 테스트를 수행했습니다.

응답:

```text
HTTP/2 401
{"ok":false,"error":"authentication_required","status":401,"message":"API key does not have access to this resource"}
```

결론:

- Outline REST API에는 API key 사용 가능
- Outline MCP endpoint에는 API key 사용 불가
- Passkey는 사람 로그인용 WebAuthn 자격 증명이므로 MCP client 인증에 직접 사용할 수 없음
- 따라서 현재 앱에서 Outline MCP를 붙이려면 `authorization_code + PKCE` 구현이 필요함

현재 앱에서 시도하지 말아야 하는 방식:

- `OAuth Client Credentials`
- `Bearer Token`에 Outline API key 입력
- Passkey 사용

향후 필요 작업:

- 브라우저 기반 OAuth 로그인 시작
